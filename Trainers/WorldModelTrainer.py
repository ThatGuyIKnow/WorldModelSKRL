...
class WorldModelSequentialTrainer(Trainer):
    def __init__(self,
                 env: Wrapper,
        ...
        self.world_model = _cfg.models['world_model']
    ...
    def train(self) -> None:
        ...
        latent = self.world_model.initial_state()
        for timestep in tqdm.tqdm(range(self.initial_timestep, self.timesteps), disable=self.disable_progressbar):
            ...
            # compute actions
            with torch.no_grad():
                latent, h_state = torch.vstack([self.world_model.predict(states[scope[0]:scope[1]], latent) \
                                               for scope in self.agents_scope])
                actions = torch.vstack([agent.act(state, timestep=timestep, timesteps=self.timesteps)[0] \
                                        for agent, state in zip(self.agents, h_state)])
            ...
            with torch.no_grad():
                if terminated.any() or truncated.any():
                    states, infos = self.env.reset()
                    latent = self.world_model.intial_state()
                else:
                    states = next_states
